...
import { caption1, caption1Strong, caption2, subtitle2 } from '../type'
import { CARD } from './chartTokens'
...
function Swatch({ color, dashed, rule }: Omit<LegendItem, 'label'>) {
  if (rule) {
...
export function StackedBar({
  ...
  width: number | string
  height: number | string
  segments: readonly { f: number; bg: string }[]
  opacity?: number
}) {
  return (
    <div
...
export function MultiLine({
  ...
  series: readonly { color: string; solid?: string; dashed?: string }[]
}) {
  return (
    <svg
      viewBox="0 0 100 100"
...
export function ShareBar({
  ...
  width: number | string
  segments: readonly { h: number | string; bg: string }[]
  opacity?: number
}) {
  return (
    <div
