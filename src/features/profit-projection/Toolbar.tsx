...
import { StatusPill } from '../../ds/components/Button'
import { Menu, MenuItem } from '../../ds/components/Overlay'
import { useHover } from '../../ds/useHover'
import { body1, body1Strong } from '../../ds/type'
import { DAYS, GRAINS, WEEK_LABEL } from './data'
import { badge, statusOf } from './calc'
...
function Stepper({
  ...
  const arrow = (dir: 'prev' | 'next', enabled: boolean, onClick: () => void) => (
    <StepArrow dir={dir} enabled={enabled} onClick={onClick} />
  )
  return (
    <div
      style={{
...
function PlainButton({ children, onClick }: { children?: ReactNode; onClick?: () => void }) {
  const [hover, hoverProps] = useHover()
  return (
...
export function Toolbar({ s }: { s: ProjectionState }) {
