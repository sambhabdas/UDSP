...
import { body1, caption1, caption1Strong, caption2Strong, subtitle2 } from '../../ds/type.js'
import {
  DAY_COLUMNS,
  DAY_TOTALS,
  DAYS,
...
function HeadCell({ def, sort, onSort }) {
...
export function DetailTable({ s }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ maxHeight: isWeek ? 'none' : 400, overflow: 'auto', display: 'flex', flexDirection: 'column' }}>
        {built.rows.map((r, i) => (
...
function MoreRow({ children, onClick }) {
  const [hover, hoverProps] = useHover()
  return (
    <div
      onClick={onClick}
...
function weekRows(s, defs) {
  if (s.empty) {
    return {
      rows: [],
      more: null,
      totals: [{ def: defs[0], t: 'No days in range', c: 'var(--text-helper)' }],
    }
  }
  const wk = weekTotals()
...
function dayRows(s, defs) {
  ...
  const rows = sorted.map((p) => {
    ...
    return { cells, bg: amber ? 'var(--warning-bg)' : 'transparent', onClick: null }
  })
