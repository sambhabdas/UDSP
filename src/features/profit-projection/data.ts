...
/** One day of the week: the editable grain of this page. */
...
export type Grain = 'Day' | 'Week' | 'Pay period' | 'Custom'

export const GRAINS: Grain[] = ['Day', 'Week', 'Pay period', 'Custom']

/** One line of the payroll split. */
...
export const DAY3_BREAKDOWN: CostLine[] = [
...
/** A payroll row for one person on one day. `pos` is null when the code did
 *  not match anyone on the roster. */
...
export const PEOPLE: Person[] = DRAWN_PEOPLE.concat(
...
/** One day in the import preview, and what importing it would do. */
...
export const SPLIT_BASE: SplitRow[] = [
...
export const SPLIT_EXTRA: SplitRow[] = ([
...
] as [string, number, string][]).map(([day, people, gross], i) => ({
  day,
  people,
  gross,
  ...(i < 7 ? NEW_DAY : REPLACES),
}))
...
export const SEED_NOTES_WEEK: Note[] = [
...
export const SEED_NOTES_DAY: Record<number, Note[]> = {
