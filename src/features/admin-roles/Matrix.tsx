...
import { BANDS, FULL, POSTS, PUNCH, READ } from './data'
...
const CAP_COL: CSSProperties = { flex: 2.2, minWidth: 220 }
...
const PILLS: Partial<Record<CellValue, Pill>> = {
...
function columnBg(i: number, selIdx: number): string {
  if (i === selIdx) return 'var(--blue-50)'
  if (i === 0 && selIdx < 0) return 'var(--surface-subtle)'
  return 'transparent'
}

function Cell({ value, i, selIdx }: { value: CellValue; i: number; selIdx: number }) {
...
function Band({ band, rows, selIdx }: { band: string; rows: CapRow[]; selIdx: number }) {
  return (
...
export function Matrix({ selIdx }: { selIdx: number }) {
  return (
