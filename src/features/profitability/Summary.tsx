...
/** What `delta` returns: the formatted move and its signed size. */
type Delta = { text: string; d: number }
import { Gauge } from '../../ds/charts/ChartKit'
import { useHover } from '../../ds/useHover'
import { caption1, caption1Strong, subtitle1, title3 } from '../../ds/type'
import { labelEyebrow as EYEBROW } from '../../ds/type'
import { ALL, CURRENT } from './data'
import { basis, badge, delta, der, fmtRange, money, otOf, pct } from './calc'
...
export function CurrentStrip({ s }: { s: ProfitabilityState }) {
...
function ViewCurrent({ onClick }: { onClick: () => void }) {
...
const deltaColor = (d: Delta | null, invert?: boolean): string | undefined =>
  d === null ? undefined : (invert ? d.d <= 0 : d.d >= 0) ? 'var(--success-fg)' : 'var(--danger-fg)'

function DeltaLine({ d, invert, vs }: { d: Delta | null; invert?: boolean; vs: string }) {
  return (
...
export function MoneyKpis({ s }: { s: ProfitabilityState }) {
...
export function WhatMovesIt({ s }: { s: ProfitabilityState }) {
  const sel = s.selected
  const d = der(sel)
  const b = basis(sel, s.compare)
  const vs = b ? b.label : s.compare === 'none' ? 'Comparison off' : 'No previous period'
  ...
  const gaugeColor = (dl: Delta | null) => (dl ? (dl.d <= 0 ? 'var(--success-accent)' : 'var(--danger-accent)') : 'var(--neutral-400)')
