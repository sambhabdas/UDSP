...
import { ALL, PERIODS } from './data.js'

const r2 = (v) => Math.round(v * 100) / 100

export const money = (v) =>
  '$' + v.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })
export const money0 = (v) => '$' + Math.round(v).toLocaleString('en-US')
...
export function fmtRangeNum(start) {
  const end = addDays(start, 13)
...
// is still moving.
export function basis(sel, mode) {
...
export function delta(cur, base, kind) {
...
export const heat = (v, lo, hi, invert) =>
  scaleColor(hi === lo ? 1 : invert ? 1 - (v - lo) / (hi - lo) : (v - lo) / (hi - lo))

export const rangeOf = (arr) => [Math.min(...arr), Math.max(...arr)]

// Tooltip anchor, clamped so it never runs off either edge of the plot.
export const colLeft = (i, n) => `${Math.min(88, Math.max(12, ((i + 0.5) / n) * 100))}%`
