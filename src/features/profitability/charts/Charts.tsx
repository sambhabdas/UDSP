...
// Every chart draws the same window of periods against the same state.
interface ChartProps {
  s: ProfitabilityState
  periods: Period[]
}

// The hours charts only ever draw periods that carry timecards.
...
function Labels<P extends Period>({
  periods,
  sel,
  marginLeft = 46,
  marginRight = 0,
...
const colBg = (p: Period, sel: string) => (p.id === sel ? 'var(--blue-50)' : 'transparent')

const hooks = (s: ProfitabilityState, chart: string, i: number, p: Period) => ({
  onEnter: () => s.setTip({ chart, i }),
  onLeave: () => s.setTip(null),
  onClick: () => s.pickPeriod(p.id),
})

const tipIdx = (s: ProfitabilityState, chart: string) =>
  s.tip && s.tip.chart === chart ? s.tip.i : null

// --- c1 · Revenue and gross profit -----------------------------------------

export function RevenueAndProfit({ s, periods }: ChartProps) {
    ...
    >
      <div style={{ display: 'flex', gap: 'var(--size-80)' }}>
        ...
        <Plot height={H}>
          {periods.map((p, i) => {
            const d = der(p)
            const bar = (v: number, color: string): CSSProperties => ({
              width: 12,
...
export function PerRouteEconomics({ s, periods }: ChartProps) {
  const n = periods.length
  const active = tipIdx(s, 'c3')
  const series = (get: (d: Derived) => number, color: string) => {
    ...
    >
      <div style={{ display: 'flex', gap: 'var(--size-80)' }}>
        ...
        <Plot height={H}>
          {periods.map((p, i) => {
            const d = der(p)
            const at = (v: number) => `${((v / AXIS.perRoute) * 100).toFixed(1)}%`
            return (
              <Column
                key={p.id}
...
export function RevenueDistribution({ s, periods }: ChartProps) {
  const n = periods.length
  const active = tipIdx(s, 'c2')
  const shares = (p: Period) => ({
...
export function CostBreakdown({ s, periods }: ChartProps) {
...
export function TimecardHours({ s, periods }: TimedChartProps) {
...
export function RegularVsOvertime({ s, periods }: TimedChartProps) {
...
export function WorkforceUtilization({ s, periods }: ChartProps) {
