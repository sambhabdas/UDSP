...
import { ALL, RESTATED_TITLE, WARN_TITLE } from './data.js'
import {
  badge,
  der,
  driverBand,
...
function LinkText({ children, onClick, style }) {
  const [hover, hoverProps] = useHover()
  return (
    <span
      onClick={onClick}
...
function FilterRow({ label, on, onClick }) {
  const [hover, hoverProps] = useHover()
  return (
    <div
      onClick={onClick}
...
function PanelButton({ children, onClick, primary }) {
  const [hover, hoverProps] = useHover()
  return (
    <div
      onClick={onClick}
