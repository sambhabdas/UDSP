...
import { addDays, fmtD, generatePeriods } from './calendar'
...
export interface FigureSource {
  ...
  by: string
  on: string
  /** Anything the uploader wrote alongside the figures. */
  note?: string
}
...
export type YearStatus = 'locked' | 'empty'
...
/** A generated-but-uncommitted calendar for a year. */
...
export type ManualEntry = Record<'dg' | 'dt' | 'pg' | 'pt' | 'tg' | 'tt', string>
...
export const STATUS_NAME: Record<PeriodStatus, string> = {
  posted: 'Posted',
  uploaded: 'Uploaded',
...
export const STATUS_TONE: Record<PeriodStatus, readonly [string, string, string, string]> = {
...
export const MANUAL_GROUPS: readonly (readonly [string, keyof ManualEntry, keyof ManualEntry])[] = [
  ['Driver', 'dg', 'dt'],
  ['Dispatch', 'pg', 'pt'],
  ['Training', 'tg', 'tt'],
]
...
export function seedPeriodStates(rows: PeriodRow[]): Record<number, PeriodState> {
  const states: Record<number, PeriodState> = {}
  rows.forEach((r) => {
    if (r.end >= TODAY) {
...
export function initialYears(): Record<number, YearState> {
