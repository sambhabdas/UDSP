...
import { Field } from '../../ds/components/Overlay'
...
import { fmtRange, money, periodWeeks } from './calendar'
import { MANUAL_GROUPS, TODAY, UNMAPPED_ROWS, UNMAPPED_TOTAL } from './data'
...
function UploadZone({ s }: { s: PayrollState }) {
...
function ManualEntry({ s }: { s: PayrollState }) {
  if (!s.periodRow) return null
  return (
    <div
      style={{
        ...card,
...
function FiguresTable({ s }: { s: PayrollState }) {
  const fig = s.figures
  if (!fig) return null
