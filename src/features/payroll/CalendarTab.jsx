...
import { Button, IconButton, StatusPill } from '../../ds/components/Button.jsx'
import { Field, Menu, MenuItem } from '../../ds/components/Overlay.jsx'
import { useHover } from '../../ds/useHover.js'
import { body1, body1Strong, caption1, caption1Strong, caption2Strong } from '../../ds/type.js'
import { fmtD, fmtRange, fromIso, iso, periodWeeks, weekOf } from './calendar.js'
import { CAL_FILTERS, STATUS_NAME, STATUS_TONE, TODAY } from './data.js'
...
function SortHeader({ label, sortKey, sort, setSort, style, title }) {
...
function PeriodRow({ row, state, isPreview, edit, setEdit, commitEdit, onOpen }) {
  const [hover, hoverProps] = useHover()
...
function WeeksLink({ label, title, onClick }) {
  const [hover, hoverProps] = useHover()
  return (
    <span
      onClick={onClick}
...
export function CalendarTab({ s }) {
  ...
  const statusOf = (r) => s.periodStates[r.n] || { status: 'empty' }
  const calName = (n) => (n === 'empty' ? 'Not started' : STATUS_NAME[n])
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--size-40)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--size-80)' }}>
                ...
                {seedD && (
                  ...
                  >
                    {seedValid ? `W${seedWeek.n} · ${seedWeek.y}` : 'Not a Sunday'}
                  </StatusPill>
                )}
              </div>
            </div>
...
function CalFilter({ s, rows, calName, statusOf }) {
  const [hover, hoverProps] = useHover()
  const applied = s.calFilter !== 'All'
  const count = (f) => rows.filter((r) => calName(statusOf(r).status) === f).length
  return (
