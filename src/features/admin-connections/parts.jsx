...
import { body1, caption1, caption1Strong, caption2, caption2Strong, subtitle2 } from '../../ds/type.js'
...
export function Card({ children, pad = 'var(--size-200)', gap = 'var(--size-160)', overflow }) {
  return (
    <div
      style={{
...
export function Field({ value, onChange, placeholder, type, inputMode, sample, width, strong }) {
  return (
    <span
      data-field=""
      style={{
...
export function ChipRow({ options, value, onPick, labelOf }) {
  return (
...
export function Chip({ label, on, onPick }) {
...
export function SmallButton({ children, onClick, primary, danger, height = 28 }) {
  const [hover, hoverProps] = useHover()
...
export function TestResult({ children }) {
  return (
...
export function Select({ label, open, onToggle, width = 150, children, muted }) {
  const [hover, hoverProps] = useHover()
  const [focus, focusProps] = useFocusRing()
  return (
...
export function Dropdown({ width, left = 0, top = 31, flip, children }) {
  return (
    <div
...
export function OptionRow({ children, on, onPick, trailing }) {
  const [hover, hoverProps] = useHover()
  return (
    <div
      onClick={onPick}
...
export function RowMenu({ open, onToggle, items, flip, width = 56, minWidth = 190 }) {
  const [hover, hoverProps] = useHover()
  return (
...
export function SearchField({ value, onChange, placeholder, width = 260 }) {
  return (
    <span
      data-field=""
      onClick={(e) => e.stopPropagation()}
...
export function HeadCell({ h, sort, onSort }) {
  const [hover, hoverProps] = useHover()
  const [focus, focusProps] = useFocusRing()
...
export function Modal({ title, width = 560, onClose, children }) {
  return (
    <div
      onClick={onClose}
      style={{
...
export function ModalFoot({ children, lead }) {
  return (
...
export function Btn({ children, onClick, tone, disabled }) {
  const [hover, hoverProps] = useHover()
...
export function Helper({ children, color = 'var(--text-helper)' }) {
