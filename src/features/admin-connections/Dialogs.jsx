import { caption1 } from '../../ds/type.js'
import { ASSIGN_KINDS, DELETE_BODY, IMAP, INFO, RELEASE_BODY, SYNC_WINDOWS, USER_POOL } from './data.js'
...
export function AddLineDialog({ s }) {
  const state = { number: s.al.number, query: s.al.numQuery, area: s.al.area, areaQuery: s.al.areaQuery }
  const patch = (p) => {
    const map = { number: 'number', query: 'numQuery', area: 'area', areaQuery: 'areaQuery' }
    const out = {}
...
export function MailConnectDialog({ s }) {
  const m = s.mailForm
...
export const MAIL_IMAP = IMAP

// ---- delete a line --------------------------------------------------------

export function DeleteLineDialog({ s }) {
...
export function ReserveDialog({ s }) {
  return (
...
export function ReleaseDialog({ s }) {
  return (
