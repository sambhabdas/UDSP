...
import type { ContactsState, SortColumn } from './useAdminContacts'
import { Icon } from '../../ds/icons/Icon'
import { useHover } from '../../ds/useHover'
import { body1, caption1, caption1Strong, caption2, caption2Strong } from '../../ds/type'
import { CheckBox } from './ReasonGroups'
import { CHIP_LIMIT, EXPORT_FORMATS } from './data'

/** A directory column. `k` is the sort key, or null when the column is not sortable. */
interface Head {
  k: SortColumn | null
  label: string
  w?: string
...
const cellBox = (h: Head): CSSProperties => ({
...
const smallControl = (hover: boolean): CSSProperties => ({
  boxSizing: 'border-box',
  height: 28,
  display: 'flex',
  alignItems: 'center',
...
function Row({
  ...
  const [hover, hoverProps] = useHover()
  // flatMap rather than map+filter: an id with no reason behind it drops out
  // and what is left is a list of reasons, not a list of maybe-reasons.
  const held = c.reasons.flatMap((rid) => s.reasons.find((x) => x.id === rid) ?? [])
...
function EditLink({ children, onClick }: { children?: ReactNode; onClick?: () => void }) {
  const [hover, hoverProps] = useHover()
  return (
    <div
      onClick={onClick}
...
function RowMenu({ c, s, flip }: { c: Contact; s: ContactsState; flip: boolean }) {
...
function MenuTrigger({ onClick }: { onClick: (e: MouseEvent<HTMLSpanElement>) => void }) {
  const [hover, hoverProps] = useHover()
  return (
    <span
      onClick={onClick}
...
function ReasonFilter({ s }: { s: ContactsState }) {
...
export function Directory({ s }: { s: ContactsState }) {
  ...
  if (!manual) {
    const key = (c: Contact) => (s.sort.col === 'title' ? c.title : c.who)
