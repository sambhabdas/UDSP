...
import { ReasonGroups } from './ReasonGroups.jsx'
import { REASONS_HELP } from './data.js'

export function InfoDot({ title, size = 14 }) {
  return (
    <span
      title={title}
      style={{
...
export function ToneButton({ children, onClick, tone }) {
...
export function SearchField({ value, onChange, placeholder, flex }) {
  return (
    <span
      data-field=""
      style={{
...
export function CategorySelect({ label, width, open, onToggle, cats, current, onPick, newValue, onNewChange, onNewCommit, isDuplicate, align = 'right' }) {
  const [hover, hoverProps] = useHover()
  return (
...
function CatRow({ label, selected, onClick }) {
  const [hover, hoverProps] = useHover()
  return (
    <div
      onClick={onClick}
...
export function ReasonsPanel({ s }) {
  ...
  const dupCat =
    s.vcNew.trim() && s.allCategories.some((c) => c.toLowerCase() === s.vcNew.trim().toLowerCase())
