...
export interface Reason {
  ...
  urgent: boolean
  retired: boolean
  /** Which group the reason sits under on the driver's Help screen. */
  cat: string
}
...
export interface Contact {
  ...
  title: string
  phone: string
  /** Whether a driver sees this contact at all. */
  shown: boolean
  reasons: number[]
}

const CATEGORY_OF: Record<number, string> = {
...
].map<Reason>((r) => ({ ...r, retired: false, cat: CATEGORY_OF[r.id] || 'Other' }))
