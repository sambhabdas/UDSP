...
/** One entry in a person's timeline. `k` is what kind of thing happened. */
export type ActivityKind = 'date' | 'in' | 'out' | 'call' | 'email' | 'note' | 'sys'
...
/** The Recents preview line: which channel, what it said, how long ago. */
...
const person = (id: string, name: string, extra: Partial<Person> = {}): Person => ({
  id,
  name,
  role: `DA · Station ${env.stationCode}`,
...
export type Channel = 'Texts' | 'Calls' | 'Emails' | 'Notes' | 'System'

// The channel chips map onto activity kinds; `All` clears the selection.
export const CHANNEL_KINDS: Record<Channel, ActivityKind[]> = {
  Texts: ['in', 'out'],
  Calls: ['call'],
  Emails: ['email'],
  Notes: ['note'],
...
export type QueueFilter = 'All' | 'Unread' | 'Missed'
export type ComposerTab = 'Text' | 'Email' | 'Note'

export const FILTERS: QueueFilter[] = ['All', 'Unread', 'Missed']
export const CHANNELS: (Channel | 'All')[] = ['All', 'Texts', 'Calls', 'Emails', 'Notes', 'System']
