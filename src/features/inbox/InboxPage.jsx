...
import { RecentsPanel } from './RecentsPanel.jsx'
import { ActivityPanel } from './ActivityPanel.jsx'
import { DecisionRail } from './DecisionRail.jsx'
import { useInbox } from './useInbox.js'

// The universal dialer toggle — it belongs to Inbox but sits on every page.
// Closed 📞 ↔ open ✕; the widget itself is specified on the Dialer page.
function DialerFab({ open, onToggle }) {
  const [hover, hoverProps] = useHover()
  return (
    <div
      onClick={onToggle}
      title={open ? 'Close dialer' : 'Open dialer'}
      style={{
        position: 'absolute',
        right: 'var(--size-200)',
        bottom: 'var(--size-200)',
      ...
      {...hoverProps}
    >
      <Icon name={open ? 'IbDismiss' : 'IbCallFilled'} size={18} />
    </div>
  )
}
...
export function InboxPage({ onRoadStale = false }) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ActivityPanel
          ...
          composer={{
            ...
            onSend: state.send,
            onCall: () => state.setDialerOpen(true),
          }}
        />
      ...
      <DialerFab
        open={state.dialerOpen}
        onToggle={() => state.setDialerOpen(!state.dialerOpen)}
      />
    </div>
  )
}
