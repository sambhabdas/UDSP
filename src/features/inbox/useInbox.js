...
import { CHANNEL_KINDS, seedPeople } from './data.js'
...
export function useInbox() {
  ...
  const [drafts, setDrafts] = useState({}) // per person, per user (§3.3)
  ...
  const patchSelected = useCallback(
    (fn) => setPeople((list) => list.map((p) => (p.id === selectedId ? fn(p) : p))),
    [selectedId],
  )
  ...
  const setDraft = useCallback(
    (value) => setDrafts((d) => ({ ...d, [selectedId]: value })),
    [selectedId],
  )
  ...
  const send = useCallback(() => {
    ...
    setPeople((list) => {
      ...
      return [sent, ...next.filter((p) => p.id !== selectedId)]
    })
    setDraft('')
    setSubject('')
  }, [draft, tab, subject, selectedId, setDraft])

  const toggleChannel = useCallback((c) => {
    setChannels((sel) =>
