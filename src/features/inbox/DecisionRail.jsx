...
import { StatusChip } from '../../ds/components/Chip.jsx'
...
import { initials, tint } from './data.js'
...
const divider = { height: 1, background: 'var(--border-subtle)' }

const jumpLink = { alignSelf: 'flex-end', ...caption1, whiteSpace: 'nowrap' }

function ContactRow({ glyph, bg, border, color, children }) {
  return (
...
function RouteCard({ person, stale }) {
  return (
    <div style={card}>
...
function PerformanceCard({ person }) {
...
function GhostButton({ children, onClick, gap }) {
...
function TasksCard({ person, state }) {
...
export function DecisionRail({ person, stale, isDrawer, open, onClose, state }) {
