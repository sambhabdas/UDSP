...
import { DA_HEADS, DA_TONE, INVITE_STAMP, WHY } from './data'
import {
  CheckBox,
  FilterButton,
  HeadCell,
...
export function DaTab({ s }: { s: UsersState }) {
  const kpis = [
    ...
    <>
      ...
      <TableCard>
        ...
        <TableScroll minWidth={900}>
          <HeadRow>
            {DA_HEADS.map((h) => (
              <HeadCell key={h.label} h={h} sort={s.daSort} onSort={() => h.k && s.daSortBy(h.k)} />
            ))}
          </HeadRow>
...
function DaRow({ d, s, flip }: { d: Da; s: UsersState; flip: boolean }) {
...
function DaBulkBar({ s }: { s: UsersState }) {
