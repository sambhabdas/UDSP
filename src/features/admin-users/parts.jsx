...
import { body1, caption1, caption1Strong, caption2, subtitle1, subtitle2 } from '../../ds/type.js'
import { EYEBROW, FOCUS_RING, useFocusRing } from './ui.js'

export function CheckBox({ on, onClick, dim }) {
  return (
    <span
      onClick={onClick}
      style={{
...
export function FilterButton({ applied, onClick }) {
...
export function SmallButton({ children, onClick, primary, icon }) {
  const [hover, hoverProps] = useHover()
  const [focus, focusProps] = useFocusRing()
  return (
    <div
...
export function ToneButton({ children, onClick, tone, trailing }) {
  const [hover, hoverProps] = useHover()
  return (
    <div
      onClick={onClick}
...
export function Row({ children, fg }) {
  const [hover, hoverProps] = useHover()
  return (
    <div
      style={{
...
export function RowMenu({ open, onToggle, items, minWidth = 210, flip }) {
  const [hover, hoverProps] = useHover()
  return (
...
export function ZeroState({ text, onClear }) {
  return (
...
export function Labelled({ label, children, width, flex, min }) {
  return (
