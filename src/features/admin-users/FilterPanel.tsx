...
export function FilterPanel({ s }: { s: UsersState }) {
  ...
  const sections = s.filterDefs
    .map((d) => {
      // Widened to string[]: the three sections hold different literal unions and
      // the panel only ever compares them against its own option values.
      const picked: string[] = s.draft ? s.draft[d.key] : []
...
function ClearAll({ enabled, onClick }: { enabled: boolean; onClick: () => void }) {
  const [hover, hoverProps] = useHover()
  const [focus, focusProps] = useFocusRing()
  return (
    <span
