...
import type { Da, DaSortKey, DaState, ImportFor, Role, User, UserSortKey, UserStatus } from './data'

/** The three multi-selects the filter drawer edits. */
...
/** One multi-select section of the filter drawer. */
...
const EMPTY_FILTERS: Filters = { roleFilter: [], statusFilter: [], daFilter: [] }
const EMPTY_FORM: UserForm = { editId: null, email: '', name: '', mobile: '', role: 'Operations' }

const cap = (s: string) => s.charAt(0).toUpperCase() + s.slice(1)

export function useAdminUsers() {
  const [tab, setTab] = useState<'portal' | 'da'>('portal')
  ...
  const [sort, setSort] = useState<{ col: UserSortKey; dir: 'asc' | 'desc' }>({
    col: 'name',
    dir: 'asc',
  })
  const [daSort, setDaSort] = useState<{ col: DaSortKey; dir: 'asc' | 'desc' }>({
    col: 'state',
    dir: 'asc',
  })

  const [sel, setSel] = useState<number[]>([])
  const [daSel, setDaSel] = useState<number[]>([])

  // Portal rows key by id; DA rows key by `da<id>`, so the two tabs cannot
  // open each other's menus.
  ...
  const [importFor, setImportFor] = useState<ImportFor>('portal')
  const [importFile, setImportFile] = useState('')
  const [daInviteOpen, setDaInviteOpen] = useState(false)
  const [daInviteSel, setDaInviteSel] = useState<number[]>([])
  ...
  const toggleDraft = useCallback((key: FilterKey, value: string) => {
    setDraft((d) => {
      if (!d) return d
  ...
  const visibleUsers = useMemo(() => {
    ...
    const keyOf = (u: User): string | number =>
  ...
  const visibleDas = useMemo(() => {
    ...
    const keyOf = (d: Da): string | number =>
  ...
  const sortBy = useCallback((k: UserSortKey) => {
    setSort((s) => ({ col: k, dir: s.col === k ? (s.dir === 'asc' ? 'desc' : 'asc') : 'asc' }))
  }, [])

  const daSortBy = useCallback((k: DaSortKey) => {
  ...
  const patchForm = useCallback((p: Partial<UserForm>) => setForm((f) => ({ ...f, ...p })), [])
  ...
  const setStatus = useCallback((ids: number[], status: UserStatus, message: string) => {
  ...
  const setRole = useCallback((ids: number[], role: Role) => {
    const n = ids.length
  ...
  const commitTransfer = useCallback(() => {
    if (!transferReady || !transferPick) return
    setUsers((us) =>
      us.map((u) =>
        u.role === 'Owner'
          ? { ...u, role: 'Sub Admin' as Role }
          : u.id === transferPick?.id
            ? { ...u, role: 'Owner' as Role }
            : u,
      ),
    )
  ...
  // ---- DA actions ---------------------------------------------------------
  const setDaState = useCallback((ids: number[], patch: Partial<Da>, message: string) => {
  ...
  const toggleDaSel = useCallback((id: number) => {
  ...
  const toggleDaInvite = useCallback((d: Da) => {
    if (!d.phone) {
  ...
  // ---- import -------------------------------------------------------------
  const openImport = useCallback((forWhat: ImportFor) => {
