...
import { body1, caption1, caption2 } from '../../ds/type'
import { INVOICE_HEADS, INVOICE_STATUS_TONE, YEAR_CHIPS } from './data'
import {
  Section,
  StatusPill,
} from './parts'
...
export function Invoices({ s }: { s: BillingState }) {
  return (
...
function YearChip({ label, on, onPick }: { label: string; on: boolean; onPick: () => void }) {
...
function InvoiceRow({ v, s, flip }: { v: Invoice; s: BillingState; flip: boolean }) {
  const [hover, hoverProps] = useHover()
...
function NumberCell({ v, onOpen }: { v: Invoice; onOpen: () => void }) {
  const [hover, hoverProps] = useHover()
  return (
    <div
      onClick={onOpen}
...
function RowMenu({ v, s, flip }: { v: Invoice; s: BillingState; flip: boolean }) {
  const [hover, hoverProps] = useHover()
