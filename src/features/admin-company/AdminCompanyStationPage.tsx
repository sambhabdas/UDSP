...
import { body1, caption1, caption1Strong, caption2, subtitle2, subtitle2Stronger } from '../../ds/type'
import { BrandPreview } from './BrandPreview'
import { DEFAULT_DEFS, INFO, TZ_CONFIRM, WEEK_STARTS } from './data'
import { Card, ChipRow, Field, Helper, InfoDot, Labelled, Section, SmallButton } from './parts'
import { MONO, SUB_EYEBROW } from './ui'
import { useCompanyStation } from './useCompanyStation'
...
export function AdminCompanyStationPage() {
  ...
  return (
    ...
    >
      ...
      {s.tzPending !== null && <TzConfirm s={{ ...s, tzPending: s.tzPending }} />}
...
function DefaultCell({ label, info, children }: { label: string; info: string; children?: ReactNode }) {
  return (
    <div
      style={{
        boxSizing: 'border-box',
...
function TimeZonePicker({ s }: { s: CompanyState }) {
  const [hover, hoverProps] = useHover()
  return (
...
function ZoneRow({ label, on, onPick }: { label: string; on: boolean; onPick: () => void }) {
  const [hover, hoverProps] = useHover()
  return (
    <div
      onClick={onPick}
