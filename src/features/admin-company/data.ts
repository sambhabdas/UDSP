...
export const SEED: CompanyForm = {
  legalName: env.companyLegalName,
...
export const SAMPLE_FIELDS: Partial<Record<keyof CompanyForm, string>> = {
...
export interface DefaultDef {
  key: 'dateFmt' | 'currency' | 'distance'
  label: string
  info: string
  options: string[]
  sample: (v: string) => string
}

export const DEFAULT_DEFS: DefaultDef[] = [
  {
    key: 'dateFmt',
    label: 'Date format',
    info: 'Presentation only. Applies product-wide including exports.',
    options: ['Mon, Jul 29, 2026', '07/29/2026'],
    sample: (v: string) => `Sample: ${v}`,
  },
  {
    key: 'currency',
    label: 'Currency',
  ...
  },
  {
    ...
    sample: (v: string) => `Sample: 85,000 ${v === 'Miles' ? 'mi' : 'km'}`,
  },
]
...
export const TZ_CONFIRM = (zone: string) =>
